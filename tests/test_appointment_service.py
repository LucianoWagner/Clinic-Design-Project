...
from app.schemas.appointment import PatientInput
...
def test_hold_and_confirm_appointment(session: Session) -> None:
    ...
    interaction = session.get(InteractionSession, 1)
    patient = service.identify_or_create_patient(
        PatientInput(full_name="Juan Perez", document_number="12345678", phone="1122334455")
    )
